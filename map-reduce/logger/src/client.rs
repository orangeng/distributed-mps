...
fn main() {
    ...
    for (idx, host) in HOSTS.iter().enumerate() {
        ...
        let mut f = File::open(HEARTBEAT_FILE).unwrap();
        ...
        // Define address
        let dest_addr: String = (*host).to_owned() + ":" + PORT;
        ...
        loop {
            // Read size bytes
            let size: i32 = payload_size(&mut stream);
            if size == 0 {
                break;
            }
